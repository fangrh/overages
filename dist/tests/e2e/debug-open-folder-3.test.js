...
test('open folder flow - API integration test', async ({ page }) => {
    ...
    console.log('Monaco:', await page.evaluate(() => !!window.monaco));
    console.log('File select value:', await page.$eval('#file-select', el => el.value));
    ...
    console.log('Python files:', filesData.files?.filter((f) => f.endsWith('.py')));
