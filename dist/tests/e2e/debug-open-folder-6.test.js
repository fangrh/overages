...
test('full flow: open folder -> file select populated -> click file -> editor loads', async ({ page }) => {
    ...
    console.log('Studio initialized:', await page.evaluate(() => !!window.studio));
    // Set workspace path
    ...
    console.log('Python files:', result.files?.filter((f) => f.endsWith('.py')));
    ...
    const uiResult = await page.evaluate(async (dir) => {
        ...
        const res = await fetch('/api/files');
        const data = await res.json();
        const pyFiles = data.files?.filter((f) => f.endsWith('.py')) || [];
